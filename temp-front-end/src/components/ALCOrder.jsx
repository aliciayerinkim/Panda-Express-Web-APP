                      ...
                      <Card key={item.getProductName} onClick={() => addALC(item.getProductName, item.getMenuItemId, item.getSalePrice)} className='d-inline-flex mb-2' style={{cursor: "pointer", padding: "10px 0px 10px 0px", boxShadow: "rgb(0 0 0 / 16%) 1px 1px 5px"}}>
                        <Row>
                          <Col xs={3} className="my-auto">
                            <img src={Images[item.getProductName.split(' ').join('')] ? Images[item.getProductName.split(' ').join('')] : Images.PandaExpressLogo} alt="" className="img-fluid"/>
                          </Col>
                        ...
                        <Col xs={7} md={8} className='text-white text-left'>
                            <div className="my-3 ml-4">
                                ...
                                <h6 style={{fontSize: '14px'}}>
                                    {(alc.length === 0) ? translatedTextList[3] : `${alc[0].name}`}
                                </h6>
                            </div>
                        </Col>
                      ...
                      <div id="example-collapse-text" >
                          ...
                          <div className="text-center" style={{width: "100%", backgroundColor: "#f7f7f6", borderTop: "1px solid #cbcbcb"}}>
                            <Row className='d-inline-flex my-3' style={{width: "100%"}}>
                              ...
                              <Col>
                                <Button disabled={(alc.length === 0)} variant="panda-btn" size="md" className="mt-2 bg-red text-white" onClick={() => checkALC()} style={{fontWeight: 800, width: "100%"}}>{translatedTextList[4]}</Button>
                              </Col>
                            </Row>
                          </div>
                      </div>
                ...
                <Col>
                  <Row>
                    ...
                    <Card key={item.getProductName} onClick={() => addALC(item.getProductName, item.getMenuItemId, item.getSalePrice)} style={{cursor: "pointer", width: '14rem', boxShadow: "rgb(0 0 0 / 16%) 1px 1px 10px"}} className="mx-2 mb-2">
                      <Card.Img variant="top" src={Images[item.getProductName.split(' ').join('')] ? Images[item.getProductName.split(' ').join('')] : Images.PandaExpressLogo}/>
                    ...
                    <Card.Body style={{overflowY: 'scroll'}}>
                      {
                        ...
                        <Row key={item.name} className='d-inline-flex mb-3'>
                          ...
                          <Button variant="panda-btn" size="sm" className="mt-2 bg-red text-white" onClick={() => deleteALC(item)} style={{fontWeight: 800}}>{translatedTextList[5]}</Button>
                          </Col>
                        </Row>))
                      }
                    </Card.Body>
                    <Card.Footer style={{height: '4rem'}}>
                      <Row>
                        ...
                        <Col className="text-right  my-auto">
                          <Button disabled={(alc.length === 0)} variant="panda-btn" size="md" className="mt-1 mr-2 bg-red text-white" onClick={() => checkALC()} style={{fontWeight: 800}}>{translatedTextList[4]}</Button>
                        </Col>
                      </Row>
                    </Card.Footer>
                  </Card>
